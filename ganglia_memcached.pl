...
# modifications to include gets and sets by geoff papilion 
#############################################################
use strict;
...
# we need separate files for each port for $time_diff to not be 0 or a few seconds
$tmp_stats_file.=$port;
...
# Memcache Hit Ratio
#################################################################################
my $hits   = $stats{'total'}->{'get_hits'} || 0;
my $misses = $stats{'total'}->{'get_misses'} || 0;
...
#
  ...
  $gets = $stats { 'hosts' }{ $host }{ 'misc' }->{ 'cmd_get' };
  $sets = $stats { 'hosts' }{ $host }{ 'misc' }->{ 'cmd_set' };
}
...
print "memcache_gets: " . $gets . "\n";
print "memcache_sets: " . $sets . "\n";
...
sub write_stats_file () {
	...
	print NEWSTATUS "cmd_get=" . $gets . "\n";
	print NEWSTATUS "cmd_set=" . $sets . "\n";
...
if ( ! -f $tmp_stats_file ) {
	...
	######################################################
 	...
 	while(<OLDSTATUS>)
 	{
        ...
        # added some new stats, don't want to log in everywhere.
        # this just adds the current value is it doesn't exist. 	
        if (! $old_stats{'cmd_get'} ) {
           $old_stats{'cmd_get'}  = $gets;
        }

        if (! $old_stats{'cmd_set'} ) {
           $old_stats{'cmd_set'}  = $sets;
        }
	...
	if ( $debug == 0 ) {
	    ...
	    if ( $bytes_read >= $old_stats{'bytes_read'} ) {
	      ...
	      system($gmetric_command . " -u bytes/s -tfloat -n memcache_bytes_written".$metric_suffix." -v " . eval( ($bytes_written - $old_stats{'bytes_written'}) / $time_diff ) );
              system($gmetric_command . " -u gets/s -tfloat -n memcache_gets".$metric_suffix." -v " . eval( ($gets - $old_stats{'cmd_get'}) / $time_diff ) );
              system($gmetric_command . " -u sets/s -tfloat -n memcache_sets".$metric_suffix." -v " . eval( ($sets - $old_stats{'cmd_set'}) / $time_diff ) );
	    } 
	    write_stats_file;
